...
function getresult(status, content) {
  ...
  if (status) {
    ...
    const responseBody = 'Unable to upload the shipperAccount';
    response = {
      statusCode: 403,
      headers: {
        'Content-Type': 'application/json',
...
async function ddbupdate(id, firstname, lastname, phone, email) {
  let statusCode = 0;
  const params = {
...
module.exports.postShipper = async (event) => {
  const {
